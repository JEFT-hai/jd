...
function renderSlider(elem,len,num,allNum,btn,indi,lk){
	...
	if(btn){
		html += '<div class="slider_wrapper" style="opacity:1;width:'+(allNum/num + 2)*len+'px;-ms-transform:translate('+(-len)+'px,0);transform:translate('+(-len)+'px,0);transition:none">';
		...
		html += '<div class="slider_wrapper" style="opacity:1;width:'+(allNum/num)*len+'px;-ms-transform:translate(0,0);transform:translate(0,0);transition:none">';
	...
	if(btn){
		...
		bind(slider_control_next,'click',function(e){
			...
			slider_wrapper.style.msTransform = 'translate('+(-elem.index-1)*len+'px,0)';
			slider_wrapper.style.msTransition = 'all 0.5s ease-in-out';
			if(elem.index == (allNum/num)){
				elem.index = 0;
				setTimeout(function(){
					slider_wrapper.style.transform = 'translate('+(-1)*len+'px,0)';
					slider_wrapper.style.msTransform = 'translate('+(-1)*len+'px,0)';
					slider_wrapper.style.transition = 'none';
					slider_wrapper.style.msTransition = 'none';
					// if(indi){
		...
		bind(slider_control_prev,'click',function(e){
			if(!elem.index){
				elem.index = 0;
			}
			elem.index  = elem.index - 1;
	...
	if(indi){
		...
		bind(slider_i,'mouseover',function(e){
			...
			if(target.nodeName.toLowerCase() == 'i'){
				...
				if(btn){
					...
					slider_w.style.msTransform = 'translate('+(-(index + num)*len)+'px,0)';
					slider_w.style.transition = 'all 0.5s ease-in-out';
					slider_w.style.msTransition = 'all 0.5s ease-in-out';
				}else{
					slider_w.style.transform = 'translate('+(-index*len)+'px,0)';
					slider_w.style.msTransform = 'translate('+(-index*len)+'px,0)';
					slider_w.style.transition = 'all 0.5s ease-in-out';
					slider_w.style.msTransition = 'all 0.5s ease-in-out';
